...
function createAndBindVao(){
    let theVAO = gl.createVertexArray();
    gl.bindVertexArray(theVAO);
    return theVAO
}

function createAndBindVertexBuffer(shape, program){
    let vrtxBffr = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vrtxBffr);
    gl.bufferData(
        gl.ARRAY_BUFFER,
...
function fillBuffer(shape, program){
    let nrmlBffr = gl.createBuffer();
    gl.bindBuffer(gl.NORMAL_ARRAY_BUFFER, nrmlBffr);
    gl.bufferData(
...
function setIBO(shape){
    let idxBffr = gl.createBuffer();
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, idxBffr);
    gl.bufferData(
...
function bindVAO(shape, program) {
  ...
  //create and bind VAO
    let theVAO = createAndBindVao()

  // create and bind vertex buffer
    createAndBindVertexBuffer(shape,program)
...
function setUpPhong(program) {
  ...
  setLightConditions(program)
  setUniformLocation(program)

  let modelMatrix = glMatrix.mat4.create();
  
  glMatrix.mat4.scale(modelMatrix, modelMatrix, [1.2, 1.2, 1.2]);
...
function setUpCamera(program) {
  ...
  let prjtMtrx = glMatrix.mat4.create();
  glMatrix.mat4.perspective(prjtMtrx,radians(30),1.0,2,400)
  gl.uniformMatrix4fv(program.uProjT, false, prjtMtrx);



  let viewMtrx = glMatrix.mat4.create();
  glMatrix.mat4.lookAt(viewMtrx, [7, 0, 1], [0, -1, 0], [0, 1, 0]);
  gl.uniformMatrix4fv(program.uViewT, false, viewMtrx);
}


function setLightConditions(program){
    let ambientLight = glMatrix.vec3.fromValues(0.25,0.25,0.25)
    gl.uniform3fv(program.ambientLight,ambientLight)


    let lightPosition=glMatrix.vec3.fromValues(10,10,10)
...
function setUniformLocation(program){
